...
const LazySyntaxHighlighter = React.lazy(() =>
  Promise.all([
    ...
    const Comp = mod.Prism;
    const atom = style.atomDark;
    return { default: (props: any) => React.createElement(Comp, { ...props, style: atom }) };
  }),
);
// @ts-ignore
import { IconCheck, IconCopy } from "@tabler/icons-react";

type CodeBlockProps = { language: string; filename: string; highlightLines?: number[] } & (
  | { code: string; tabs?: never }
  | {
    code?: never;
    tabs: Array<{ name: string; code: string; language?: string; highlightLines?: number[] }>;
  }
);
...
export const CodeBlock = ({
  ...
  return (
    ...
    >
      ...
      <React.Suspense fallback={<div className="text-gray-400">Loading code block...</div>}>
